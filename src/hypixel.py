...
class Player:
    ...
    def __init__(self, data: dict):
        ...
        self.last_login = datetime.datetime.fromtimestamp(float(data.get('lastLogin', 0)) / 1000).strftime("%y-%m-%d %H:%M:%S")
        ...
...
class HypixelClient:
    ...
    async def fetch_election(self) -> dict:
        async with self._lock:
            async with self._session.get("https://api.hypixel.net/resources/skyblock/election", headers=self._headers, timeout=10) as response:
                if response.status == 200:
                    return await response.json()

    async def fetch_player_info(self, uuid: str) -> Player:
        async with self._lock:
            async with self._session.get("https://api.hypixel.net/player", params={'uuid': uuid}, headers=self._headers, timeout=10) as response:
                if response.status == 200:
                    return Player((await response.json())['player'])

    async def fetch_skyblock_profiles(self, uuid: str) -> SkyblockProfile:
        async with self._lock:
            async with self._session.get("https://api.hypixel.net/skyblock/profiles", params={'uuid': uuid}, headers=self._headers, timeout=10) as response:
                if response.status == 200:
    ...
    async def fetch_skyblock_profile(self, uuid: str) -> SkyblockProfile:
        async with self._lock:
            async with self._session.get("https://api.hypixel.net/skyblock/profile", params={'profile': uuid}, headers=self._headers, timeout=10) as response:
                if response.status == 200:
